var conestado = Ext.extend(Ext.grid.GridPanel,{	
		 border		: false
		,stripeRows	: true	
		,loadMask	: true
		...
		{			
			...
			this.comboFld = new Ext.form.ComboBox({	
			    ...
			    ,store          : new Ext.data.ArrayStore({
			    	id     : 'estado_desc'
			       ,fields : ['id','field']
				   ,data   : [['estado_desc','Descrição'],
				              ['estado_sigla','Sigla'],
				              ['estado_id','Identificador']]				   
			...
			this.store = new Ext.data.JsonStore({
				 url			: 'estado/listar'
				,root			: 'rows'					
				,idProperty		: 'estado_id'
				,totalProperty	: 'totalCount'
				,autoLoad		: true
				,autoDestroy	: true
				,remoteSort     : true
				,baseParams		: {
					 action	: 'estado/listar'
					,limit	: 30
				}				
				,fields:[
			...
			//super
			conestado.superclass.initComponent.call(this);
		}
		
		,initEvents: function()
		{
			//super
			conestado.superclass.initEvents.call(this);
		...
		,onDestroy: function()
		{
			conestado.superclass.onDestroy.apply(this,arguments);
			
			//destrói a janela de usuário e limpa sua referência	
			Ext.destroy(this._winEstado)
			this._winEstado = null;
		}	
		,_onBtnNovoClick: function()		
		{
			Ext.require('cad-estado',function(){
				var winEstado = new cadestado();
				winEstado.setEstadoID(0);
				winEstado.show();				
			});
			this.store.reload();
		}
		,_onBtnExcluirSelecionadosClick: function()
		{
			...
			Ext.Msg.confirm('Confirmação','Deseja mesmo excluir o(s) registro(s) selecionado(s)?',function(opt){
				...
				{
					estadoID.push( arrSelecionados[i].get('estado_id') );
				}
		...
		,_onGridRowDblClick: function( grid, rowIndex, e ) 
		{
			...
			//extrai id
			var estadoID = record.get('estado_id');
			
			Ext.require('cad-estado',function(){
				var winEstado = new cadestado();
				winEstado.setEstadoID(estadoID);
				winEstado.show();
			},estadoID);
		}
		,_onCadastroEstadoSalvarExcluir: function()
		{
			//recarrega grid
			this.store.reload();
		}	
