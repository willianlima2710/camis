var congrauinstr = Ext.extend(Ext.grid.GridPanel,{	
		 border		: false
		,stripeRows	: true	
		,loadMask	: true
		,initComponent: function()
		{
			...
			this.comboFld = new Ext.form.ComboBox({	
			    ...
			    ,store          : new Ext.data.ArrayStore({
			    	id     : 'grauinstr_desc'
				   ,fields : ['id','field']
				   ,data   : [['grauinstr_desc','Descrição'],
  			                  ['grauinstr_id','Identificador']]				   
			    })			
			})
			...
			this.store = new Ext.data.JsonStore({
				 url			: 'grauinstr/listar'
				,root			: 'rows'					
				,idProperty		: 'grauinstr_id'
				,totalProperty	: 'totalCount'
				,autoLoad		: true
				,autoDestroy	: true
				,remoteSort     : true
				,baseParams		: {
					 action	: 'grauinstr/listar'
					,limit	: 30
				}				
				,fields:[
			...
			Ext.apply(this,{
				...
				,columns:[{
					 dataIndex	: 'grauinstr_desc'
					,header		: 'Descrição'
					,width      : 300	
					,sortable   : true	
				},{
					 dataIndex	: 'grauinstr_id'
			...
			//super
			congrauinstr.superclass.initComponent.call(this);
		}
		
		,initEvents: function()
		{
			//super
			congrauinstr.superclass.initEvents.call(this);
		...
		,onDestroy: function()
		{
			congrauinstr.superclass.onDestroy.apply(this,arguments);
			
			//destrói a janela de usuário e limpa sua referência	
			Ext.destroy(this._winGrauinstr)
			this._winGrauinstr = null;
		}	
		,_onBtnNovoClick: function()		
		{
			Ext.require('cad-grauinstr',function(){
				var winGrauinstr = new cadgrauinstr();
				winGrauinstr.setGrauinstrID(0);
				winGrauinstr.show();				
			});
			this.store.reload();
		}
		,_onBtnExcluirSelecionadosClick: function()
		{
			...
			Ext.Msg.confirm('Confirmação','Deseja mesmo excluir o(s) registro(s) selecionado(s)?',function(opt){
				...
				{
					grauinstrID.push( arrSelecionados[i].get('grauinstr_id') );
				}
		...
		,_onGridRowDblClick: function( grid, rowIndex, e ) 
		{
			...
			//extrai id
			var grauinstrID = record.get('grauinstr_id');
			
			Ext.require('cad-grauinstr',function(){
				var winGrauinstr = new cadgrauinstr();
				winGrauinstr.setGrauinstrID(grauinstrID);
				winGrauinstr.show();
			},grauinstrID);
		}
		,_onCadastroGrauinstrSalvarExcluir: function()
		{
			//recarrega grid
			this.store.reload();
		}	
...
Ext.reg('e-congrauinstr',congrauinstr);