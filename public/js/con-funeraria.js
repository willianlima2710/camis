var confuneraria = Ext.extend(Ext.grid.GridPanel,{	
		 border		: false
		,stripeRows	: true	
		,loadMask	: true
		,initComponent: function()
		{
			...
			this.comboFld = new Ext.form.ComboBox({	
			    ...
			    ,store          : new Ext.data.ArrayStore({
			    	id     : 'funeraria_desc'
				   ,fields : ['id','field']
				   ,data   : [['funeraria_desc','Descrição'],
				              ['funeraria_telefone','Telefone'],
				              ['funeraria_id','Identificador']]				   
			    })			
			})
			...
			this.store = new Ext.data.JsonStore({
				 url			: 'funeraria/listar'
				,root			: 'rows'					
				,idProperty		: 'funeraria_id'
				,totalProperty	: 'totalCount'
				,autoLoad		: true
				,autoDestroy	: true
				,remoteSort     : true
				,baseParams		: {
					 action	: 'funeraria/listar'
					,limit	: 30
				}				
				,fields:[
					 {name:'funeraria_id'  	    ,type:'int'}
					,{name:'funeraria_desc'	    ,type:'string'}
					,{name:'funeraria_telefone' ,type:'string'}
				]
			});
			...
			Ext.apply(this,{
				...
				,columns:[{
					 dataIndex	: 'funeraria_desc'
					,header		: 'Descrição'
					,width      : 300	
					,sortable   : true	
				},{
					 dataIndex	: 'funeraria_telefone'
					,header		: 'Telefone'
					,width      : 70	
					,sortable   : true	
				},{
					 dataIndex	: 'funeraria_id'
			...
			//super
			confuneraria.superclass.initComponent.call(this);
		}
		
		,initEvents: function()
		{
			//super
			confuneraria.superclass.initEvents.call(this);
		...
		,onDestroy: function()
		{
			confuneraria.superclass.onDestroy.apply(this,arguments);
			
			//destr�i a janela de usu�rio e limpa sua refer�ncia	
			Ext.destroy(this._winFuneraria)
			this._winFuneraria = null;
		}	
		,_onBtnNovoClick: function()		
		{
			Ext.require('cad-funeraria',function(){
				var winFuneraria = new cadfuneraria();
				winFuneraria.setFunerariaID(0);
				winFuneraria.show();				
			});
			this.store.reload();
		}
		,_onBtnExcluirSelecionadosClick: function()
		{
			...
			Ext.Msg.confirm('Confirmação','Deseja mesmo excluir o(s) registro(s) selecionado(s)?',function(opt){
				...
				{
					funerariaID.push( arrSelecionados[i].get('funeraria_id') );
				}
				
				this.el.mask('Excluindo');
				
				Ext.Ajax.request({
					 url	: 'funeraria/excluir'
					,params	: {
						 action	          : 'excluir'
		...
		,_onGridRowDblClick: function( grid, rowIndex, e ) 
		{
			...
			//extrai id
			var funerariaID = record.get('funeraria_id');
			
			Ext.require('cad-funeraria',function(){
				var winFuneraria = new cadfuneraria();
				winFuneraria.setFunerariaID(funerariaID);
				winFuneraria.show();
			},funerariaID);
		}
		,_onCadastroFunerariaSalvarExcluir: function()
		{
			//recarrega grid
			this.store.reload();
		}	
...
Ext.reg('e-confuneraria',confuneraria);