var conhistorico = Ext.extend(Ext.grid.GridPanel,{	
		 border		: false
		,stripeRows	: true	
		,loadMask	: true
		...
		{			
			...
			this.comboFld = new Ext.form.ComboBox({	
			    ...
			    ,store          : new Ext.data.ArrayStore({
			    	id     : 'historico_desc'
				   ,fields : ['id','field']
				   ,data   : [['historico_desc','Descri��o'],
				              ['historico_id','Identificador']]				   
			    })			
			})
			...
			//super
			conhistorico.superclass.initComponent.call(this);
		}
		
		,initEvents: function()
		{
			//super
			conhistorico.superclass.initEvents.call(this);
		...
		,onDestroy: function()
		{
			conhistorico.superclass.onDestroy.apply(this,arguments);
			
			//destr�i a janela de usu�rio e limpa sua refer�ncia	
			Ext.destroy(this._winHistorico)
			this._winHistorico = null;
		}	
		...
		,_onBtnExcluirSelecionadosClick: function()
		{
			...
			Ext.Msg.confirm('Confirma��o','Deseja mesmo excluir o(s) registro(s) selecionado(s)?',function(opt){
				...
				{
					historicoID.push( arrSelecionados[i].get('historico_id') );
				}
		...
		,_onGridRowDblClick: function( grid, rowIndex, e ) 
		{
			...
			//extrai id
			var historicoID = record.get('historico_id');
			
			Ext.require('cad-historico',function(){
				var winHistorico = new cadhistorico();
				winHistorico.setHistoricoID(historicoID);
				winHistorico.show();
			},historicoID);
		}
		,_onCadastroHistoricoSalvarExcluir: function()
		{
			//recarrega grid
			this.store.reload();
		}	
...
Ext.reg('e-conhistorico',conhistorico);