var conreligiao = Ext.extend(Ext.grid.GridPanel,{	
		 border		: false
		,stripeRows	: true	
		,loadMask	: true
		...
		{		
			...
			this.comboFld = new Ext.form.ComboBox({	
			    ...
			    ,store          : new Ext.data.ArrayStore({
			    	id     : 'religiao_desc'
				   ,fields : ['id','field']
				   ,data   : [['religiao_desc','Descrição'],
				              ['religiao_id','Identificador']]				   
			    })			
			})
			...
			this.store = new Ext.data.JsonStore({
				 url			: 'religiao/listar'
				,root			: 'rows'					
				,idProperty		: 'religiao_id'
				,totalProperty	: 'totalCount'
				,autoLoad		: true
				,autoDestroy	: true
				,remoteSort     : true
				,baseParams		: {
					 action	: 'religiao/listar'
					,limit	: 30
				}				
				,fields:[
			...
			Ext.apply(this,{
				...
				,columns:[{
					 dataIndex	: 'religiao_desc'
					,header		: 'Descrição'
					,width      : 300	
					,sortable   : true	
				},{
					 dataIndex	: 'religiao_id'
			...
			//super
			conreligiao.superclass.initComponent.call(this);
		}
		
		,initEvents: function()
		{
			//super
			conreligiao.superclass.initEvents.call(this);
		...
		,onDestroy: function()
		{
			conreligiao.superclass.onDestroy.apply(this,arguments);
			
			//destrói a janela de usuário e limpa sua referência	
			Ext.destroy(this._winReligiao)
			this._winReligiao = null;
		}	
		,_onBtnNovoClick: function()		
		{
			Ext.require('cad-religiao',function(){
				var winReligiao = new cadreligiao();
				winReligiao.setReligiaoID(0);
				winReligiao.show();				
			});
			this.store.reload();
		}
		,_onBtnExcluirSelecionadosClick: function()
		{
			...
			Ext.Msg.confirm('Confirmação','Deseja mesmo excluir o(s) registro(s) selecionado(s)?',function(opt){
				...
				{
					religiaoID.push( arrSelecionados[i].get('religiao_id') );
				}
		...
		,_onGridRowDblClick: function( grid, rowIndex, e ) 
		{
			...
			//extrai id
			var religiaoID = record.get('religiao_id');
			
			Ext.require('cad-religiao',function(){
				var winReligiao = new cadreligiao();
				winReligiao.setReligiaoID(religiaoID);
				winReligiao.show();
			},religiaoID);
		}
		,_onCadastroReligiaoSalvarExcluir: function()
		{
			//recarrega grid
			this.store.reload();
		}	
...
Ext.reg('e-conreligiao',conreligiao);