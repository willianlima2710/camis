var concaixasin = Ext.extend(Ext.grid.GridPanel,{	
		 border		: false
		,stripeRows	: true	
		,loadMask	: true
		...
		{		
			...
			this.txTotalDeb =  new Ext.form.NumberField({
			...
			this.formTotais = new Ext.Panel({
			    ...
			    ,title  : 'Totais:'			    	 	
 			    ,items  : [{
 			    	xtype      : 'fieldset'
 			       ,autoHeight : true
				   ,items      : [this.txSaldoAnt,this.txTotalCred,this.txTotalDeb,this.txTotalSald]							  
			    }]
			});		
			...
			//super
			concaixasin.superclass.initComponent.call(this);
		}

		,initEvents: function()
		{
			//super
			concaixasin.superclass.initEvents.call(this);
			
			this.store.on('load',function(store) {
				this.txTotalCred.setValue(store.sum('valor_Credada').toFixed(2));
				this.txTotalDeb.setValue(store.sum('valor_Deba').toFixed(2));
				this.txSaldoAnt.setValue(store.getAt(0).get('saldo_valor').toFixed(2));
			},this);			
		}	
		,onDestroy: function()
		{
			concaixasin.superclass.onDestroy.apply(this,arguments);
		}		
		...
		{			
			this.store.reload({
				...
				,callback: function(records,option,success) {					
					...
					var totalDeb = this.txTotalDeb.getValue();
					var totalgeral = (saldoant+totalCred)-totalDeb;
		...
		{			
			var win = new Ext.Window({
			   ...
			   ,scope	    : this
			   ,maximized   : false
			   ,title		: 'Movimento de caixa - sintetico'
			   ,layout		: 'fit'
			   ,autoLoad    : {
				   url     : 'caixa/impsintetico'
				  ,params  : {
					  banco_id             : this.comboBanco.getValue()
 				     ,caixa_data_movto_ini : this.dateCaixa_data_movto_ini.getValue()
					 ,caixa_data_movto_fim : this.dateCaixa_data_movto_fim.getValue()
...
Ext.reg('e-concaixasin',concaixasin);