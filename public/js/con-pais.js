var conpais = Ext.extend(Ext.grid.GridPanel,{	
		 border		: false
		,stripeRows	: true	
		,loadMask	: true
		,initComponent: function()
		{
			...
			//super
			conpais.superclass.initComponent.call(this);
		}
		
		,initEvents: function()
		{
			//super
			conpais.superclass.initEvents.call(this);
		...
		,onDestroy: function()
		{
			conpais.superclass.onDestroy.apply(this,arguments);
			
			//destrói a janela de usuário e limpa sua referência	
			Ext.destroy(this._winPais)
			this._winPais = null;
		}	
		,_onBtnNovoClick: function()		
		{
			Ext.require('cad-pais',function(){
				var winPais = new cadpais();
				winPais.setPaisID(0);
				winPais.show();				
			});
			this.store.reload();
		...
		,_onBtnExcluirSelecionadosClick: function()
		{
			...
			Ext.Msg.confirm('Confirmação','Deseja mesmo excluir o(s) registro(s) selecionado(s)?',function(opt){
				...
				{
					paisID.push( arrSelecionados[i].get('pais_id') );
				}
		...
		,_onGridRowDblClick: function( grid, rowIndex, e ) 
		{
			...
			//extrai id
			var paisID = record.get('pais_id');
			
			Ext.require('cad-pais',function(){
				var winPais = new cadpais();
				winPais.setPaisID(paisID);
				winPais.show();
			},paisID);
		}
		,_onCadastroPaisSalvarExcluir: function()
		{
			//recarrega grid
			this.store.reload();
		}	
