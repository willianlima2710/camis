...
class Model_Function_Rcbvenda extends FPDF
{	
    ...
    public function Detail($dadosLocatario,$dadosVenda,$dadosPdsv,$dadosFormarec)
    {
    	...
    	$this->SetXY(150,44);
    	$this->Cell(0,0,$funcoes->moedabraGeral($dadosVenda['venda_total']),0,1,'R');
    	...
    	// A importancia
    	$total = trim($funcoes->valorporextenso($dadosVenda['venda_total']));
    	$this->SetXY(10,56);    	
    	...
    	for($i=0;$i<count($dadosFormarec);$i++){    		
    		$formarec = $funcoes->databrGeral($dadosFormarec[$i]['venda_rec_data_vencto']).'/'.
    		            $funcoes->moedabraGeral($dadosFormarec[$i]['venda_rec_valor']).'/'.
    		            $dadosFormarec[$i]['formarec_desc'];
    	...
    	$this->Cell(0,4,$dadosVenda['jazigo_codigo'],0,0,'L');
    	...
    	$this->SetXY(10,74);
    	for($i=0;$i<count($dadosPdsv);$i++){    		
    		$this->Cell(0,3,$dadosPdsv[$i]['prodserv_desc'].' - '.
    		                $funcoes->moedabraGeral($dadosPdsv[$i]['venda_pdsv_total']),0,1,'L');    		    		
    	}
    	...
    	$this->Cell(0,4,str_pad($dadosVenda['venda_documento'],6,'0',STR_PAD_LEFT),0,0,'C');
